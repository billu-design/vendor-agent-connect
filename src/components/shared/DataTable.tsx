...
export function DataTable({
  ...
  const filteredData = data.filter(record => Object.values(record).some(value => value && value.toString().toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  return <div className="space-y-4">
      ...
      <div className="border rounded-lg overflow-hidden">
        {isLoading ? <div className="p-8 text-center text-muted-foreground">Loading data...</div> : <Table>
            <TableHeader>
              <TableRow>
                {columns.map(column => <TableHead key={getColumnKey(column) || columns.indexOf(column).toString()} onClick={() => column.sortable && handleSort(getColumnKey(column))} className={column.sortable ? "cursor-pointer hover:text-primary" : ""}>
            ...
            <TableBody>
              {paginatedData.length === 0 ? <TableRow>
                ...
                </TableRow> : paginatedData.map((record, index) => <TableRow key={record.id || index} className="group hover:bg-muted/50">
                    {columns.map((column, colIndex) => <TableCell key={`${record.id || index}-${getColumnKey(column) || colIndex}`}>
                        {column.cell ? column.cell({
                row: {
                  original: record
                }
              }) : column.render ? column.render(getCellValue(record, column), record) : isStatusColumn(column) ? <StatusBadge status={getCellValue(record, column)} /> : getCellValue(record, column)}
                      </TableCell>)}
      ...
      {totalPages > 1 && <div className="flex justify-between items-center">
